...
namespace bid
{
    class computeMD5andSHA1
    {
        ...
        private string HashFile(string fileName, string algName)
        {
            ...
            HttpWebRequest HttpWReq = (HttpWebRequest)WebRequest.Create(fileName);
            HttpWebResponse HttpWResp = (HttpWebResponse)HttpWReq.GetResponse();
            //HttpWResp.
            System.IO.Stream fs = HttpWResp.GetResponseStream();
            byte[] hashBytes = HashData(fs, algName);
            fs.Close();
            HttpWResp.Close();
            return ByteArrayToHexString(hashBytes);
        }
        private string HashText(string text, string algName)
        {
            //if (!System.IO.File.Exists(fileName))
            //    return string.Empty;

            //System.IO.FileStream fs = new System.IO.FileStream(fileName, System.IO.FileMode.Open, System.IO.FileAccess.Read);
            byte[] stream = System.Text.Encoding.Unicode.GetBytes(text);
        ...
        private byte[] HashData(byte[] stream, string algName)
        {
            System.Security.Cryptography.HashAlgorithm algorithm;
            if (algName == null)
            {
