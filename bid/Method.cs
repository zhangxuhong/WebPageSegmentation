...
namespace bid
{
    static class Method
    {
        ...
        public static string getHtmlPath(HtmlElement child, HtmlElement root)
        {
            List<string> layoutElements = new List<string> { "TABLE", "TBODY", "TH", "TR", "TD", "LI", "UL", "OL", "DL", "DD", "DT", "DIV" };
            HtmlElement current = child;
            string path = "";
            while (!current.Equals(root))
            {
                //path = "/" + current.TagName + "[" + Method.getIndex(current, current.Parent) + "]" + path;
                if (layoutElements.Contains(current.TagName))
                {
                    path = "/" + current.TagName + path;
                }
                current = current.Parent;
            }
            path = root.TagName + path;
            //if (root.TagName.Equals("BODY"))
            //{
            //    path = "/HTML" + path;
            //}
            return path;
        ...
        public static int ComputeLineNumber(List<TreeNode> children)
        {
            ...
            for (int i = 0; i < children.Count; i++)
            {
                temp = (IHTMLElement)((DataNode)children[i].Tag).DomNode.DomElement;
                if (temp.offsetHeight != 0)
                {
                    top = Method.getElementTop(temp);
